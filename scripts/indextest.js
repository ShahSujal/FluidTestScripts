...
const main = async () => {
    try {
        ...
        console.log("\n--- Test 1: Create Agent (off-chain) ---");
        const agent = sdk.createAgent("Test Agent", "A test agent for FluidSDK", "ipfs://QmTestImageHash");
        ...
        try {
            const reputationResult = await sdk.searchAgentsByReputation(undefined, // agents
            undefined, // tags
            undefined, // reviewers
        ...
        if (!sdk.isReadOnly) {
            ...
            console.log("\n--- Test 8: Prepare Feedback ---");
            if (TEST_AGENT_ID) {
                const feedbackFile = sdk.prepareFeedback(TEST_AGENT_ID, 5, // score
                ["test", "positive"], // tags
                "Great agent!", // text
                "chat", // capability
                "Test Agent", // name
