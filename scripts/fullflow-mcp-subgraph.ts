...
const main = async () => {
  try {
    ...
    const agentDescription =
      "Test agent with MCP server integration for full flow testing";
    ...
    try {
      ...
      let mcpTools: string[] = [];
      let mcpPrompts: string[] = [];
      let mcpResources: string[] = [];
      ...
      try {
        ...
        if (toolsResponse.ok) {
          ...
          if (toolsData.result?.tools) {
            mcpTools = toolsData.result.tools.map((t: any) => t.name);
            console.log(
              `   ✅ Found ${mcpTools.length} tool(s): ${mcpTools.join(", ")}`
            );
          }
        }
      } catch (e) {
        console.log(
          "   ⚠️  Could not fetch tools:",
      ...
      try {
        ...
        if (promptsResponse.ok) {
          ...
          if (promptsData.result?.prompts) {
            mcpPrompts = promptsData.result.prompts.map((p: any) => p.name);
            console.log(
              `   ✅ Found ${mcpPrompts.length} prompt(s): ${mcpPrompts.join(
                ", "
              )}`
            );
          }
      ...
      try {
        ...
        if (resourcesResponse.ok) {
          ...
          if (resourcesData.result?.resources) {
            mcpResources = resourcesData.result.resources.map(
              (r: any) => r.uri || r.name
            );
            console.log(
              `   ✅ Found ${
                mcpResources.length
              } resource(s): ${mcpResources.join(", ")}`
            );
          }
        }
      } catch (e) {
        console.log(
          "   ⚠️  Could not fetch resources:",
          e instanceof Error ? e.message : String(e)
        );
      }
      ...
      if (
        ...
        console.log(
          `   Prompts: ${mcpPrompts.length} - ${mcpPrompts.join(", ")}`
        );
        console.log(
          `   Resources: ${mcpResources.length} - ${mcpResources.join(", ")}`
        );
      }
